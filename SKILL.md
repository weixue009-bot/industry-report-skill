...
# 通用（在 skill 目录下运行，适用于 .zip 安装用户）：
cd ~/.workbuddy/skills/industry-report && python scripts/collect_reports.py \
  --mode industry \
  --industry "{行业名}" \
  --segments "{环节1,环节2,环节3}" \
  --months {月数} \
...
- `{环节名}.json`：该环节专属研报列表
- `industry_stats.json`：同花顺 hithink-industry-query 采集的行业结构化数据（估值/行情/板块排名等），供 Step 4 AI 分析作为评分量化依据

**四个数据源架构**：
采集脚本为**多源平级架构**——研报三源 + 行业统计数据源：

| 数据源 | 认证 | 类型 | 特点 |
|--------|------|------|------|
| **iwencai report-search** | 需 `IWENCAI_API_KEY` | 研报搜索 | 按环节关键词精准搜索，带摘要+正文节选 |
| **东方财富 reportapi** | 免费公开 | 研报搜索 | 按行业分类过滤，覆盖广、免费 |
| **发现报告 fxbaogao.com** | 免费公开 | 研报搜索 | 覆盖最广（单行业百篇+），支持相对时间过滤 |
| **hithink-industry-query（官方 skill）** | 需 `IWENCAI_API_KEY` | 行业统计数据 | 行业估值/财务/行情/板块排名结构化数据 |

- **研报三源**都可用时同时从三个源拉取，**自动合并去重**（按 uid + title+date 联合去重）；部分不可用时自动降级
- **hithink-industry-query**：独立管道，在研报采集完成后自动追加行业统计数据到 `industry_stats.json`
...
**额外读取：行业统计数据**：同时读取 `raw/industry/{行业}/industry_stats.json`（由 Step 3 的 hithink-industry-query 自动采集）。该文件包含：

| 数据 | 字段路径 | 用途 |
|------|---------|------|
| 行业整体估值 | `valuation[0]` 中的行业市盈率、市净率 | "估值位置"评分量化依据 |
| 板块资金动向 | `momentum[]` 中的资金净流入额、主力净买入额 | "资金动向"评分量化依据 |
| 各环节代表个股 | `seg_valuation[环节名]` 中的毛利率、营收同比、PE | 子页评分参考 |

**重要要求**：在研判 `sector_scores` 的"估值位置"和"资金动向"维度时，必须引用 `industry_stats.json` 中的实际数据作为支撑。示例：
```json
{"name": "估值位置", "score": 65, "note": "机器人板块 PE(TTM)约-65（整体亏损），PB 7.08× 处于历史中位"}
{"name": "资金动向", "score": 78, "note": "板块资金净流入 12.79 亿，主力净买入 16.02 亿，资金活跃度较高"}
```
...
```json
{
  ...
  "segments": [
    // ⚠️ 每个 segment 必须包含以下 14 个字段，缺一不可。
    // 模板 renderSegment() 依序渲染 9 张卡片：
    //   首屏三卡片 → 竞争格局(两栏) → 产品分层 → 壁垒 → 失败条件
    //   → 利润驱动 → 技术路线图 → 评分维度+标的 → 证据来源
    // 缺少任意字段会导致子环节页面不完整。
    {
      "name": "环节名",
...
**⚠️ Segment 全字段清单**：每个 segment 至少要有以下字段，缺的卡片不会渲染：

| 卡片（模板渲染顺序） | 对应字段 | 必填 |
|------|------|:--:|
| 首屏三卡片（定位+价值量+国产化） | `positioning` `value_ratio` `localization_progress` | ✅ |
| 国际/国内竞争格局 | `international_landscape` `domestic_landscape` | ✅ |
| 产品分层 | `product_tiers` | ✅ |
| 壁垒类型 | `barrier_type` `barrier_detail` | ✅ |
| 失败条件 | `failure_conditions` | ✅ |
| 利润驱动 | `profit_drivers` | ✅ |
| 技术路线图 | `tech_roadmap` | ✅ |
| 评分维度+标的 | `scoring_dimensions` `stocks` | ✅ |
| 证据来源 | `evidence_sources` | ✅ |

> 之前创新药报告因缺少 `product_tiers`/`profit_drivers`/`tech_roadmap`/`scoring_dimensions`/`evidence_sources` 导致子环节页 VS 机器人报告少 5 张卡片。详细写法见 `references/format.md` 各字段专节。
...
# 通用（.zip 安装用户，在 skill 目录下）：
cd ~/.workbuddy/skills/industry-report && python scripts/build_report.py \
  --mode industry \
  --data "analysis/industry/{行业}/analysis.json" \
  --output "output/industry/{行业}_产业研究报告.html"
```
...
```bash
# 本地环境：
PYTHONIOENCODING=utf-8 python C:\Users\001\.workbuddy\skills\industry-report\scripts\company_collect.py --code "{代码}" --output-dir "C:\Users\001\Desktop\研报研究"

# 通用（在 skill 目录下运行）：
cd ~/.workbuddy/skills/industry-report && PYTHONIOENCODING=utf-8 python scripts/company_collect.py --code "{代码}" --output-dir "."
```
...
```bash
# 本地环境：
PYTHONIOENCODING=utf-8 python C:\Users\001\.workbuddy\skills\industry-report\scripts\company_analysis.py --code "{代码}" --work-dir "C:\Users\001\Desktop\研报研究"

# 通用（在 skill 目录下运行）：
cd ~/.workbuddy/skills/industry-report && PYTHONIOENCODING=utf-8 python scripts/company_analysis.py --code "{代码}" --work-dir "."
```
