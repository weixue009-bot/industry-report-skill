...
对每个环节，用 `WebFetch` 打开该环节最新 3~5 篇研报的 URL 获取原文内容。如 URL 无法访问，则使用 API 返回的 `summary` 字段。
...
| 研报 URL 无法访问 | 使用 API 返回的 summary 字段，在报告中注明"内容基于摘要分析" |
...
| 文件 | 功能 |
|------|------|
| `scripts/collect_reports.py` | **双数据源采集**：iwencai report-search + 东方财富 reportapi 双源合并去重 |
